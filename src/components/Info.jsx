        ...
        <div className="mt-12">
            ...
            <form onSubmit={handleSubmit} className="w-full">
              <div className="relative">
                <input
                  ...
                  type='text'
                  id="default-search"
                  className=" block p-4 w-full  pl-6 text-sm text-gray-900 border font-bold active:border-purple-400 rounded-2xl bg-purple-50 dark:bg-[#1F1F1F]"
                  placeholder="Search..."
                  required
                />
          ...
          <div>
            <div className="flex justify-between mt-[45px]">
              <div >
                <h1 className="font-bold text-6xl dark:text-white">{word}</h1>
            ...
            <div className="flex items-center mt-10">
              <p className="font-bold text-2xl italic dark:text-white">{meanings[0].partOfSpeech}</p>
              <div className="w-full h-[1px] bg-[#E9E9E9] ml-5"></div>
            </div>
            <div className="mt-10">
              <h1 className="font-normal text-xl text-[#757575] dark:text-[#757575]" >Meaning</h1>
              <ul className="mt-[25px] line list-disc marker:text-[#8F19E8] ml-[47px]">
                {meanings[0].definitions.map((item) => {
                  return (<li key={Math.random()} className="font-normal text-lg  mt-[13px] dark:text-white">
                    {item.definition}
                  </li>)
                })}
          ...
          {meanings[1] && <div>
            <div className="flex items-center mt-10">
              <p className="font-bold text-2xl italic dark:text-white">{meanings[1].partOfSpeech ? meanings[1].partOfSpeech : 'none'}</p>
              <div className="w-full h-[1px] bg-[#E9E9E9] ml-5"></div>
            </div>
            <div className="mt-10">
              ...
              <ul className="mt-[25px] line list-disc marker:text-[#8F19E8] ml-[47px]">
                {meanings[1].definitions.map((item) => {
                  return (<li key={Math.random()} className="font-normal text-lg  mt-[13px] dark:text-white">
                    {item.definition} <br />
                    <span className="text-[#757575]">{item.example ? item.example : ''}</span>
                  </li>
                  )
                })}
              </ul>
          ...
          <div className="border-t-[#E9E9E9] border-t-[1px] mt-10 flex">
            <a className="mt-[19px] flex underline font-normal text-sm	decoration-[#757575] dark:text-white" href={sourceUrls[0]} ><span className="text-[#757575] mr-5">Sourse</span>{sourceUrls[0]}<img className="ml-2" src={linkIcon} alt="" /></a>
          </div>
        </div >
      );
    }
